...
{
		    NRF_LOG_INFO("write_i_i2c start");
    NRF_LOG_FLUSH();

    memset(am_rx_buf, 0, am_length);

		    NRF_LOG_INFO("write_i_i2c 0");
    NRF_LOG_FLUSH();
	
    nrf_gpio_pin_write(OLED_DC, LOW);

		    NRF_LOG_INFO("write_i_i2c 1");
    NRF_LOG_FLUSH();
	nrf_delay_us(200);
    nrf_gpio_pin_write(OLED_CS, LOW);

		    NRF_LOG_INFO("write_i_i2c 2");
    NRF_LOG_FLUSH();
	nrf_delay_us(200);
    APP_ERROR_CHECK(nrf_drv_twi_tx(&m_twi, OLED_ADDRESS, &i, 1, true));

		    NRF_LOG_INFO("write_i_i2c 3");
    NRF_LOG_FLUSH();
	nrf_delay_us(300);
    nrf_gpio_pin_write(OLED_CS, HIGH);

		    NRF_LOG_INFO("write_i_i2c end");
    NRF_LOG_FLUSH();
}	
...
void oled_init_i2c(void)
{
    ...
    twi_init_oled();
    NRF_LOG_INFO("twi_init_oled end");
    NRF_LOG_FLUSH();
