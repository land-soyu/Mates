...
void nrf_pwm_set_enabled(bool enabled)
{
    ...
    else
    {
        ...
        {
            nrf_gpio_pin_clear(pwm_gpiote_channel[i]);
        ...
        if(pwm_modified[i])
        {
            ...
            else
            {
                if(i < 2)
                {
                    ...
                    if(!pwm_running[i])
                    {
                        nrf_gpiote_task_configure(pwm_gpiote_channel[i], pwm_io_ch[i], NRF_GPIOTE_POLARITY_TOGGLE, NRF_GPIOTE_INITIAL_VALUE_HIGH);  
                        pwm_running[i] = 1;
